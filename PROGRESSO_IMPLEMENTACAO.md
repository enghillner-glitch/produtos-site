...
| F056 | Enviar e tratar fotos | FASE 5 - Cadastro de imoveis | concluida | 2026-06-01 | node --check app.js | Upload preservado, limite de 5 imagens validado e imagens sao redimensionadas/comprimidas para WebP antes do envio. |
...
| F060 | Bloquear contatos em textos e imagens | FASE 5 - Cadastro de imoveis | concluida | 2026-06-01 | node --check app.js; tests/static-checks.mjs | Textos publicos, nomes de arquivos e texto reconhecido por OCR nas imagens bloqueiam telefone, email e link antes do upload. |
...
| F062 | Pausar, expirar e renovar anuncio | FASE 5 - Cadastro de imoveis | concluida | 2026-06-01 | node --check app.js | Anuncios possuem validade, expiracao por RPC de manutencao e renovacao por mais 30 dias no painel do usuario. |
...
| F091 | Aplicar limites antispam | FASE 7 - Propostas | concluida | 2026-06-01 | node --check app.js; supabase.sql | RLS limita duplicidade/volume de propostas e o front aplica campo-armadilha, tempo minimo de formulario e cooldown local. |
...
| F113 | Enviar e-mail resumido de lead | FASE 9 - Lead e painel interno | concluida | 2026-06-01 | supabase.sql; api/maintenance.js; Vercel env | Eventos de lead/acordo entram na fila de email com destino do Auth e a manutencao envia via Resend; usar dominio proprio validado antes do remetente definitivo. |
...
| F121 | Encerrar cancelamento e liberar imoveis | FASE 10 - Cancelamento e retorno | concluida | 2026-06-01 | supabase.sql; tests/e2e-authenticated-backend.mjs | Administracao aprova o pedido, marca proposta como `failed`, cancela lead, cancela termos finais pendentes e reabre imoveis; RPC legado de reabertura direta foi bloqueado para participantes. |
...
| F141 | Criar e-mails automaticos | FASE 12 - Notificacoes e jobs | concluida | 2026-06-01 | api/maintenance.js; OPERACAO_JOBS.md; Vercel env | Manutencao envia `email_queue` via Resend; `RESEND_API_KEY` e `EMAIL_FROM` foram cadastrados na Vercel sem gravar segredos no repositorio. |
...
| F152 | Aplicar rate limit e CAPTCHA adaptativo | FASE 13 - Seguranca e privacidade | concluida | 2026-06-01 | node --check app.js; node --check api/verify-turnstile.js; tests/unit-turnstile.mjs | Rate limit por RLS, friccao local e Cloudflare Turnstile opcional com validacao server-side estao prontos; ativacao exige site key e secret key. |
...
| F154 | Criar backup e testar restauracao | FASE 13 - Seguranca e privacidade | concluida | 2026-06-01 | BACKUP_RESTAURACAO.md; tests/backup-restore-dry-run.mjs; scripts/backup-project.mjs | Scripts geram backup com manifest/hash e usam pg_dump/psql quando SUPABASE_DB_URL estiver disponivel; dry-run local validado sem expor segredo. |
...
| F175 | Executar homologacao funcional | FASE 15 - Testes e descontinuacao | concluida | 2026-06-01 | node --check; static-checks; unit-maintenance; unit-turnstile; backup dry-run; smoke/e2e local e producao; check-deployment-config; e2e-authenticated-backend | Homologacao automatizada executada e registrada; schema avancado aplicado no Supabase; variaveis obrigatorias e Resend cadastrados na Vercel; E2E autenticado valida proposta, contato restrito, bloqueio de atalhos fora do fluxo, cancelamento e limpeza dos dados temporarios. |
